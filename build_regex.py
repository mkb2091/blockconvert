...
class BuildRegex():
    ...
    def generate_domain_regex(self):
        with open('tld_list.txt') as file:
            self.TLDS = [tld for tld in file.read(
            ).lower().splitlines() if '#' not in tld]
        tlds = self.TLDS.copy()
        tld_dict = dict()
        for tld in tlds:
        ...
        for first_letter in sorted(tld_dict, reverse=True):
            now = []
            for second_letter in sorted(
                    tld_dict[first_letter],
                    key=lambda x: len(
                        tld_dict[first_letter][x]),
                    reverse=True):
                current = '|'.join([re.escape(i) for i in sorted(
                    tld_dict[first_letter][second_letter], key=len, reverse=True)])
    ...
    def generate_url_regex(self):
        ...
        ending = r'(?:/[/a-zA-Z0-9_?&=.%-]*)?'
        self.URL_STRING = r'{protocol}{domain_string}{ending}'.format(
            **locals())
    ...
    def generate_host_regex(self):
        ...
        domain_string = self.DOMAIN_STRING
        self.HOSTS_STRING = r'{ip_string}\s+{domain_string}\s*(?:\#.*)?'.format(
            **locals())

    def generate_adblock_regex(self):
        domain_string = self.DOMAIN_STRING
        protocol = '(?:(?:http(?:s|[*])?)|(?:[*]))?'
        url_string = r'(?:(?:{protocol}?[:])(?:(?:\/\/)|\*))?{domain_string}\/?'.format(
            **locals())
        start = r'(?:\|?\|)?'
        options = [
            'popup',
        ...
        options_other = '(?:%s)' % '|'.join(
            '(?:%s)' % i for i in ['[a-z~-]+'] + options_noop)
        options_full = r'\$(?:(?:(?:{options_other}[,])*{options_string}(?:[,]{options_other})*)|%s)'.format(
            **locals())
        options_full %= '|'.join('(?:%s)' % i for i in options_noop)
        ending = r'[*]?\|?\^?(?:{options_full})?(?:\s+(?:(?:\!.*)|(?:\#.*))?)?'.format(
            **locals())
        href_element_hiding = r'\#\#\[href\^?\=\"{url_string}\"\]'.format(
            **locals())
        self.ADBLOCK_STRING = r'(?:{start}{url_string}{ending})|(?:{href_element_hiding})'.format(
            **locals())

    def generate_dns_reponse_policy_zone_regex(self):
        domain_string = self.DOMAIN_STRING
        self.RPZ_STRING = r'{domain_string}\s+cname\s+[.]\s*(?:[;].*)?'.format(
            **locals())

    def generate_master_regex(self):
        self.REGEX_STRING = '(?:%s)|(?:%s)|(?:%s)' % (
            self.HOSTS_STRING, self.ADBLOCK_STRING, self.RPZ_STRING)
