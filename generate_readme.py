...
- Adverts: Adblock syntax is partially supported, so this list is able to extract \
some advert domains. This list is pretty good at blocking adverts, but an \
in-browser adblocker such as uBlock Origin is recommended as well as relying on \
hosts/dns blocking.
...
- uBlock Origin: Click uBlock Origin logo. Click open dashboard(3 horizontal \
lines, under disable uBlock Origin button, on the right). Click Filter lists. \
Scroll to bottom, and click Import(in custom section). Copy and paste either \
adblock style blocklist or domain blocklist from the link section below.
'''
